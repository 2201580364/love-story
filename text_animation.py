...
_SIN_NORMALIZE_SHIFT: float = 1.0  # 将 sin [-1,1] 平移到 [0,2]
_SIN_NORMALIZE_SCALE: float = 2.0  # 将 [0,2] 缩放到 [0,1]
...
class TextAnimator:
    ...
    def __init__(
        ...
        Raises:
            TypeError: 若 canvas 不是 tk.Canvas。
        ...
        # 解析颜色为 RGB 元组
        self._start_rgb: tuple[int, int, int] = _parse_hex_color(color_start)
    ...
    def update(self, global_frame: int) -> None:
        ...
        for i, char in enumerate(self._text):
            ...
            # ---- 淡入进度 ------------------------------------------------
            fade_t: float = _clamp(char_age / FADE_IN_FRAMES, 0.0, 1.0)

            # ---- 颜色（淡入 + 光晕）-------------------------------------
            # 光晕：基于 char_age 独立计时，将 sin[-1,1] 归一化到 [0,1]
            glow_t: float = (
                math.sin(_TWO_PI * char_age / GLOW_PERIOD) + _SIN_NORMALIZE_SHIFT
            ) / _SIN_NORMALIZE_SCALE  # 将 sin 值 [-1,1] 归一化到 [0,1]

            # 综合淡入进度与光晕：淡入完成前以 fade_t 为主，完成后叠加光晕脉冲
            # 光晕使 t 在 [fade_t, 1.0] 之间微微增亮
            effective_t: float = fade_t * (_clamp(1.0 - glow_t * (1.0 - fade_t), 0.0, 1.0) * 0.0 + 1.0)
            # 简化：淡入阶段以 fade_t 渐变，完成后光晕在目标色基础上脉冲
            # 重新设计：combo_t = fade_t * (1 + glow_t * (1 - fade_t) * 0.3) clamped
            combo_t: float = _clamp(fade_t + glow_t * (1.0 - fade_t) * 0.3, 0.0, 1.0)
